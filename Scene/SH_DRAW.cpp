...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	PhysicsManager* phy_manager = getGameContext()->getPhysicsManager();
	phy_manager->DebugDrawAllCollider();
    ...
    if (pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			if (hSN.isValid())
			{
				Matrix4x4 worldMatrix = hSN.getObject<SceneNode>()->m_worldTransform;
				Vector3 bounding_points[8];

				bool boundbox_in_cam = true;
				
				if (pMeshCaller->m_AABB)
				{
					boundbox_in_cam = false;
					// boundbox_in_cam = true;

					bounding_points[0] = worldMatrix * Vector3{pMeshCaller->m_AABB[0], pMeshCaller->m_AABB[2], pMeshCaller->m_AABB[4]};
					bounding_points[1] = worldMatrix * Vector3{ pMeshCaller->m_AABB[0], pMeshCaller->m_AABB[2], pMeshCaller->m_AABB[5] };
					bounding_points[2] = worldMatrix * Vector3{ pMeshCaller->m_AABB[0], pMeshCaller->m_AABB[3], pMeshCaller->m_AABB[4] };
					bounding_points[3] = worldMatrix * Vector3{ pMeshCaller->m_AABB[0], pMeshCaller->m_AABB[3], pMeshCaller->m_AABB[5] };
					bounding_points[4] = worldMatrix * Vector3{ pMeshCaller->m_AABB[1], pMeshCaller->m_AABB[2], pMeshCaller->m_AABB[4] };
					bounding_points[5] = worldMatrix * Vector3{ pMeshCaller->m_AABB[1], pMeshCaller->m_AABB[2], pMeshCaller->m_AABB[5] };
					bounding_points[6] = worldMatrix * Vector3{ pMeshCaller->m_AABB[1], pMeshCaller->m_AABB[3], pMeshCaller->m_AABB[4] };
					bounding_points[7] = worldMatrix * Vector3{ pMeshCaller->m_AABB[1], pMeshCaller->m_AABB[3], pMeshCaller->m_AABB[5] };


					DebugRenderer::Instance()->createLineBoundingBox(worldMatrix, pMeshCaller->m_AABB);


					for (int j = 0; j < 8; j++)
					{
						Vector3& point = bounding_points[j];
						bool inbox = true;
						if (pDrawEvent)
						{
							for (int k = 0; k < 6; k++)
							{
								inbox = inbox && (pDrawEvent->m_frustum[k].m_x * point.m_x + pDrawEvent->m_frustum[k].m_y * point.m_y + pDrawEvent->m_frustum[k].m_z * point.m_z + pDrawEvent->m_frustum[k].m_w) > 0.0f;
							}
						}
						else
						{
							for (int k = 0; k < 6; k++)
							{
								inbox = inbox && (pZOnlyDrawEvent->m_frustum[k].m_x * point.m_x + pZOnlyDrawEvent->m_frustum[k].m_y * point.m_y + pZOnlyDrawEvent->m_frustum[k].m_z * point.m_z + pZOnlyDrawEvent->m_frustum[k].m_w) > 0.0f;
							}
						}
						boundbox_in_cam = inbox || boundbox_in_cam;
					}

					// pInst->m_culledOut = !boundbox_in_cam;
					if (!boundbox_in_cam)
					{
