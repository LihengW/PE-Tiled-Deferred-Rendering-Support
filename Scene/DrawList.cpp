...
#include "PrimeEngine/APIAbstraction/Effect/EffectManager.cpp"
...
void DrawList::do_DEFERRED_RENDER(Events::Event* pEvt, int& threadOwnershipMask)
{
	...
	m_pCurIndBuf = 0;
	// m_pCurEffect = EffectManager::Instance()->m_hGBufferEffect.getObject<Effect>();
	m_pCurEffect = 0;
	m_pCurVertBuf = 0;
	PrimitiveTypes::Int32 curIndBufRange = -1;
	PrimitiveTypes::Int32 curIndBufRangeBoneSegment = -1;
	...
	IRenderer::checkForErrors("starting to deferred rendering\n");
	...
	for (PrimitiveTypes::UInt32 i = 0; i < m_numDrawCalls; i++)
	{
		...
		Handle h_gbuffer = EffectManager::Instance()->m_hGBufferTextureGPU_pos;
		
		// m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithNoDepth(h_gbuffer.getObject<TextureGPU>(), false);

#if PE_ENABLE_GPU_PROFILING
		Timer t;
		PE::Profiling::Profiler::Instance()->startEventQuery(Profiling::Group_DrawCalls, IRenderer::Instance()->getDevice(), t.GetTime(), dbgName);
#endif
		TextureGPU* ptr = EffectManager::Instance()->m_hGBufferTextureGPU.getObject<TextureGPU>();
		if (curDrawCallType == DrawCallType::VBUF_INDBUF && m_pCurVertBuf && m_pCurIndBuf)
		{
			if (curIndBufRange != -1)
			{
...
{
	// do_RENDER(pEvt, threadOwnershipMask);
	do_DEFERRED_RENDER(pEvt, threadOwnershipMask);
}
