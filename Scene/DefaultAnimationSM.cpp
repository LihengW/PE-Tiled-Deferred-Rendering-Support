...
void DefaultAnimationSM::do_PRE_RENDER_needsRC(PE::Events::Event *pEvt)
{
	...
	while (pSkelInst->getFirstComponentIP<MeshInstance>(index+1, index, pMeshInst))
	{
		...
		DebugRenderer::Instance()->createTextMesh((m_upperbody_Animation + "  // "  + m_downbody_Animation).c_str(), false, true, true, true, 500.0f, pSN->m_worldTransform.getPos(), 3.0f, pRealEvt->m_threadOwnershipMask);
...
void DefaultAnimationSM::do_Event_PLAY_ANIMATION(Events::Event *pEvt)
{
	...
	int walking = pRealEvent->m_walkinghalf == 1 ? 5 : 1;

	setAnimationNew(pRealEvent->m_animSetIndex, walking,
		4, 4, 5, 5,
		PARTIAL_BODY_ANIMATION | LOOPING,
		0.5f, 0, true, 0.0f,
		0, 2);

	setAnimationNew(pRealEvent->m_animSetIndex, pRealEvent->m_animIndex,
		0, 0, 1, 1,
		PARTIAL_BODY_ANIMATION | LOOPING, 
		0.5f, 0, true, 0.0f,
		3, 38); // upper half
	...
	AnimSetBufferGPU* pAnimSetBufferGPU = pSkelInstance->m_hAnimationSetGPUs[pRealEvent->m_animSetIndex].getObject<AnimSetBufferGPU>();
	AnimationSetCPU* pAnimSet = pAnimSetBufferGPU->m_hAnimationSetCPU.getObject<AnimationSetCPU>();
	AnimationCPU& anim_upper = pAnimSet->m_animations[(pRealEvent->m_animIndex + m_debugAnimIdOffset)];
	m_upperbody_Animation = anim_upper.m_name;
	AnimationCPU& anim_down = pAnimSet->m_animations[(walking + m_debugAnimIdOffset)];
	m_downbody_Animation = anim_down.m_name;
}
	...
	PrimitiveTypes::Float32 customPercentage)
{
	...
	AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset), 0, (PrimitiveTypes::Float32)(anim.m_frames.m_size-1), /*anim.m_startJoint*/ 40 , anim.m_endJoint, ACTIVE | additionalFlags /*| PARTIAL_BODY_ANIMATION | NOTIFY_ON_ANIMATION_END*/, weight);
	setSlot(goodSlot, slot);
	return &m_animSlots[goodSlot];
}

AnimationSlot* DefaultAnimationSM::setAnimationNew(
	PrimitiveTypes::UInt32 animationSetIndex,
	PrimitiveTypes::UInt32 animationIndex,
	PrimitiveTypes::UInt32 firstActiveAnimSlotIndex,
	PrimitiveTypes::UInt32 lastActiveAnimSlotIndex,
	...
	PrimitiveTypes::Bool useProgressPercentageOfReplacedAnimation,
	PrimitiveTypes::Float32 customPercentage,
	PrimitiveTypes::UInt32 start_Joint,
	PrimitiveTypes::UInt32 end_Joint)
{
	Handle hParentSkinInstance = getFirstParentByType<SkeletonInstance>();
	PEASSERT(hParentSkinInstance.isValid(), "SM has to belong to skeleton instance");
	...
	AnimationSetCPU* pAnimSet = pAnimSetBufferGPU->m_hAnimationSetCPU.getObject<AnimationSetCPU>();
	AnimationCPU& anim = pAnimSet->m_animations[(animationIndex + m_debugAnimIdOffset)];
	AnimationSlot slot(animationSetIndex, (animationIndex + m_debugAnimIdOffset), 0, (PrimitiveTypes::Float32)(anim.m_frames.m_size - 1), max<PrimitiveTypes::UInt32>(start_Joint, anim.m_startJoint), min<PrimitiveTypes::UInt32>(end_Joint, anim.m_endJoint), ACTIVE | additionalFlags /*| PARTIAL_BODY_ANIMATION | NOTIFY_ON_ANIMATION_END*/, weight);
	setSlot(goodSlot, slot);
	return &m_animSlots[goodSlot];
}
