...
void EffectManager::buildFullScreenBoard()
{
	...
	m_hGBufferEffect = getEffectHandle("GBufferWrite");
	m_hGBufferSkinEffect = getEffectHandle("GBufferWrite_Skin");
	m_hDeferredLightingEffect = getEffectHandle("DeferredLightShading");
}
...
void EffectManager::setGBufferTextureForDeferredRendering()
{
	m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithDepth(m_hGBufferTextureGPU.getObject<TextureGPU>(), m_hGBufferTextureGPU.getObject<TextureGPU>(), true, true);
	// m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithNoDepth(m_hGBufferTextureGPU.getObject<TextureGPU>(), true);
	m_pCurRenderTarget = m_hGBufferTextureGPU.getObject<TextureGPU>();
...
void EffectManager::drawDeferredLightPass()
{
	Effect& curEffect = *m_hDeferredLightingEffect.getObject<Effect>();
	if (!curEffect.m_isReady)
		return;
	...
	UpdateDeferredLightMap(m_pContext->m_pRootSceneNode->m_TiledLightMask);

	TextureGPU* t = m_hGBufferTextureGPU.getObject<TextureGPU>();

	PE::SA_Bind_Resource setAlbedoTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture0));
	setAlbedoTextureAction.bindToPipeline(&curEffect);

	PE::SA_Bind_Resource setNormalTextureAction(*m_pContext, m_arena, BUMPMAP_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture1));
	setNormalTextureAction.bindToPipeline(&curEffect);

	PE::SA_Bind_Resource setPositionTextureAction(*m_pContext, m_arena, GLOW_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture2));
	setPositionTextureAction.bindToPipeline(&curEffect);

	TextureGPU* Lightmask = m_hDefferedLightMaskTextureGPU.getObject<TextureGPU>();
	PE::SA_Bind_Resource setLightMapTextureAction(*m_pContext, m_arena, SPECULAR_TEXTURE_2D_SAMPLER_SLOT, Lightmask->m_samplerState, API_CHOOSE_DX11_DX9_OGL(Lightmask->m_pShaderResourceView, Lightmask->m_pTexture, Lightmask->m_texture));
	setLightMapTextureAction.bindToPipeline(&curEffect);
	
	// PE::SA_Bind_Resource setLightMapTextureAction(*m_pContext, m_arena, SPECULAR_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture2));
	...
	// cb.m_data.gDoMotionBlur = 0;
	// cb.m_data.gEyePosW = m_pContext->getRootSceneNode()->m_eyePos;
	// 
	// PrimitiveTypes::UInt32 iDestLight = 0;
	// if (m_pContext->getRootSceneNode()->m_lights.m_size)
	// {
	// 	for (PrimitiveTypes::UInt32 i = 0; i < (m_pContext->getRootSceneNode()->m_lights.m_size); i++) {
	// 		Light* pLight = m_pContext->getRootSceneNode()->m_lights[i].getObject<Light>();
	...
	// 			cb.m_data.gLightWVP = lightProjectionViewWorldMatrix;
	// 
	// 			cb.m_data.gLights[iDestLight] = pLight->m_cbuffer;
	// 			iDestLight++;
	// 
	// 			break;
	// 		}
	...
	m_pContext->getRootSceneNode()->m_pglobalShaderAction->bindToPipeline(&curEffect);
	...
	setAlbedoTextureAction.unbindFromPipeline(&curEffect);
	setNormalTextureAction.unbindFromPipeline(&curEffect);
	setPositionTextureAction.unbindFromPipeline(&curEffect);
	setLightMapTextureAction.unbindFromPipeline(&curEffect);

	m_pContext->getRootSceneNode()->m_pglobalShaderAction->unbindFromPipeline(&curEffect);
	// cb.unbindFromPipeline(&curEffect);
	objSa.unbindFromPipeline(&curEffect);
}

void EffectManager::UpdateDeferredLightMap(GLubyte* updateinfo)
{
	GLuint texID = m_hDefferedLightMaskTextureGPU.getObject<TextureGPU>()->m_texture;

	if (!glIsTexture(texID)) {
		std::cout << "Error: texID " << texID << " is not a valid texture!" << std::endl;
	}

	glBindTexture(GL_TEXTURE_2D, texID);
	glPixelStorei(GL_UNPACK_ALIGNMENT, 1); // Make sure byte alignment is correct

	glTexSubImage2D(
		GL_TEXTURE_2D,  // Target
		0,              // Mipmap level
		0, 0,           // xoffset, yoffset (start at top-left)
		8, 8,           // width, height
		GL_RGB,        // format
		GL_UNSIGNED_BYTE, // type
		updateinfo       // pointer to the pixel data
	);

	IRenderer::checkForErrors("glLightMask2DUpdate");
...
void EffectManager::drawMotionBlur()
{
	...
	// for deferred render
	TextureGPU* t = m_hGBufferTextureGPU.getObject<TextureGPU>();
	PE::SA_Bind_Resource setTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture2));
	// PE::SA_Bind_Resource setTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT,t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture1));
	// PE::SA_Bind_Resource setTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture2));
...
void EffectManager::drawGBuffer()
{
	Effect& curEffect = *m_hGBufferEffect.getObject<Effect>();

	if (!curEffect.m_isReady)
		return;

	m_pContext->getGPUScreen()->setRenderTargetsAndViewportWithNoDepth(m_hGBufferTextureGPU_pos.getObject<TextureGPU>(), true);

	m_pCurRenderTarget = m_hGBufferTextureGPU_pos.getObject<TextureGPU>();
	...
	// TextureGPU* t = m_hGBufferTextureGPU_pos.getObject<TextureGPU>();
	// PE::SA_Bind_Resource setTextureAction(*m_pContext, m_arena, DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, t->m_samplerState, API_CHOOSE_DX11_DX9_OGL(t->m_pShaderResourceView, t->m_pTexture, t->m_texture));
	// setTextureAction.bindToPipeline(&curEffect);
...
void EffectManager::debugDrawRenderTarget(bool drawGlowRenderTarget, bool drawSeparatedGlow, bool drawGlow1stPass, bool drawGlow2ndPass, bool drawShadowRenderTarget, bool drawGBufferDebug)
{
...
#	endif
	...
	if (drawGBufferDebug)
		setTextureAction.set(DIFFUSE_TEXTURE_2D_SAMPLER_SLOT, m_hGBufferTextureGPU_pos.getObject<TextureGPU>()->m_samplerState, API_CHOOSE_DX11_DX9_OGL(m_hGBufferTextureGPU_pos.getObject<TextureGPU>()->m_pShaderResourceView, m_hGBufferTextureGPU_pos.getObject<TextureGPU>()->m_pTexture, m_hGBufferTextureGPU_pos.getObject<TextureGPU>()->m_texture));
