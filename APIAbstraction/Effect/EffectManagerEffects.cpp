...
namespace PE {
    ...
    void EffectManager::loadDefaultEffects()
    {
		...
		PEINFO("PE: PROGRESS: Creating GBuffer Textures\n");
		// defferred rendering part
		m_hGBufferTextureGPU_pos = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU_pos = new(m_hGBufferTextureGPU_pos) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU_pos->createDrawableIntoColorTextureWithDepth(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);
		
		m_hGBufferTextureGPU_normal = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU_normal = new(m_hGBufferTextureGPU_normal) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU_normal->createDrawableIntoColorTexture(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);
		
		m_hGBufferTextureGPU_albedo = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU_albedo = new(m_hGBufferTextureGPU_albedo) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU_albedo->createDrawableIntoColorTexture(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);
		
		m_hGBufferTextureGPU_specular = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU_specular = new(m_hGBufferTextureGPU_specular) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU_specular->createDrawableIntoColorTexture(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);
		
		m_hGBufferTextureGPU_depth = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU_depth = new(m_hGBufferTextureGPU_depth) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU_depth->createDrawableIntoColorTexture(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_NoMinTexelLerp_NoMagTexelLerp_Clamp);

		m_hGBufferTextureGPU = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGBufferTextureGPU = new(m_hGBufferTextureGPU) TextureGPU(*m_pContext, m_arena);
		pGBufferTextureGPU->createGBufferRenderTarget(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);

		m_hDefferedLightMaskTextureGPU = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pDefferedLightMaskTextureGPU = new(m_hDefferedLightMaskTextureGPU) TextureGPU(*m_pContext, m_arena);
		pDefferedLightMaskTextureGPU->createDeferredLightMaskTexture(8, 8, SamplerState_NoMips_NoMinTexelLerp_NoMagTexelLerp_Clamp);
...
#	endif

	{
		...
		m_map.add("GBufferWrite", hEffect);
	}

	{
		...
		m_map.add("GBufferWrite_Skin", hEffect);
	}

	{
		PEINFO("PE: PROGRESS: Creating DeferredLighting_Effects\n");
		Handle hEffect("EFFECT", sizeof(Effect));
		Effect* pEffect = new(hEffect) Effect(*m_pContext, m_arena, hEffect);
		pEffect->loadTechnique(
			"DeferredLightShading_VS", "main",
			NULL, NULL, // geometry shader
			"DeferredLightShading_PS", "main",
			NULL, NULL, // compute shader
			PERasterizerState_SolidTriNoCull,
			PEDepthStencilState_NoZBuffer, PEAlphaBlendState_NoBlend, // depth stencil, blend states
			"DeferredLightShading");

		pEffect->m_psInputFamily = EffectPSInputFamily::REDUCED_MESH_PS_IN;

		m_map.add("DeferredLightShading", hEffect);
	}
