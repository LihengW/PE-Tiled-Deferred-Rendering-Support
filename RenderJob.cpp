...
void runDrawThreadSingleFrame(PE::GameContext &ctx)
{
	...
	if (!disableScreenSpaceEffects)
    {
		...
		if (deferredRenderer)
		{
			EffectManager::Instance()->setGBufferTextureForDeferredRendering();
		}
		else
		{
		...
		if (deferredRenderer)
		{
			DrawList::InstanceReadOnly()->do_DEFERRED_RENDER(NULL, threadOwnershipMask);
		}
		else
		{
		...
		if (drawMotionBlur)
		{
			...
			if (deferredRenderer)
			{
				EffectManager::Instance()->drawDeferredLightPass();
			}
			else
			{
		...
		else
		{
			...
			bool drawGBufferDebug = renderMode == IRenderer::RenderMode_DebugGBuffer;

			EffectManager::Instance()->debugDrawRenderTarget(debugGlowRenderTarget, drawSeparatedGlow, drawGlow1stPass, drawGlow2ndPass, drawShadowRenderTarget, drawGBufferDebug);
			EffectManager::Instance()->endCurrentRenderTarget();
		}
    }
    else
