...
float3 ParallelLightStatic(SurfaceInfo v, float3 eyePos)
{
    float3 litColor = make_float3(0, 0, 0);
	
	//Light vector (opposite photon direction) //?: + and now works
    float3 lightVec = make_float3(0, 0, -1);
	
	//Add the ambient term.
    litColor += v.diffuse.xyz * float3(0.05, 0.05, 0.05);
...
{
    if (l.xRange_yType_zw.y < -0.1)
        return float3(0.0, 0.0, 0.0);
    else if (l.xRange_yType_zw.y < 0.1) // == 0)
...
float3 DeferredRenderLight(SurfaceInfo v, Light l, float3 eyePos, float shadowFactor)
{
    if (length(v.pos) < 0.0000001f)
    {
        return float3(0.8f, 0.2f, 0.5f);
    }
	else
    {
