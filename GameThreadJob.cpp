    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
...

            {
				...
				for (int i = 0; i < 6; i++)
				{
					drawZOnlyEvt->m_frustum[i] = pcam->m_frustum[i];
				}
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				for (int i = 0; i < 6; i++)
				{
					drawEvt->m_frustum[i] = pcam->m_frustum[i];
				}

				// for (int i = 2; i < 6; i++)
				// {
				// 	drawEvt->m_frustum[i] = pcam->m_SubFrustum[7 * 4 + i - 2];
				// }

				for (int i = 0; i < 8 * 8 * 4; i++)
				{
					drawEvt->m_subfrustum[i] = pcam->m_SubFrustum[i];
				}
		...
		else if (Event_DEBUGVIEW_SWITCH::GetClassId() == pGeneralEvt->getClassId())
		{
			Event_DEBUGVIEW_SWITCH* pRealEvent = (Event_DEBUGVIEW_SWITCH*)(pGeneralEvt);
			m_pContext->m_pRootSceneNode->m_debug_state++;
			if (m_pContext->m_pRootSceneNode->m_debug_state > 5)
			{
				m_pContext->m_pRootSceneNode->m_debug_state = 0;
			}
		}
